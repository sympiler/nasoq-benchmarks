...
path1 = "SMP_Repository_test_settings_csvs/"
path2 = "SMP_Repository_tune_csvs/"
plots_path = "SMP_Repository_opt_test_plots/"
...
def compute_speedup2():
    ...
    labels1 = ["optimized\nnasoq-custom", "diag_perturb -12", "diag_perturb -11", "diag_perturb -10", "diag_perturb -9", \
    ...
    labels2 = ["optimized\nnasoq-custom", "max_iter 0", "max_iter 1", "max_iter 2", "max_iter 3", \
...
def compute_performance_profile1():
    """
...
def compute_performance_profile2():
    """
    plot the performance profile for NASOQ-CUSTOM optimized version
    ...
    def compute_helper(eps, solvers):
        ...
        for idx in range(n_problems):
            if status["optimized nasoq-custom"][idx] not in SOLUTION_PRESENT:
                t["optimized nasoq-custom"][idx] = MAX_TIMING
...
def compute_failure_rate2():
    """
    failure rate of nasoq-custom with different settings
    """
    data = os.listdir(path1)
    custom_data = []
    frate = {}
    for datum in data:
        ...
        for s in status:
            if s != 1:
                convergence_lst.append(0)
            else:
                convergence_lst.append(1)
        failure_rate = 1.0 - convergence_lst.count(1) / len(convergence_lst)
        if "eps-3" in datum:
            if "nasoq-custom" in datum:
                frate["nasoq-custom_eps-3"] = failure_rate
            elif "nasoq-tuned" in datum:
                frate["nasoq-tuned_eps-3"] = failure_rate
            elif "nasoq-fixed" in datum:
                frate["nasoq-fixed_eps-3"] = failure_rate
        elif "eps-6" in datum:
            if "nasoq-custom" in datum:
                frate["nasoq-custom_eps-6"] = failure_rate
            elif "nasoq-tuned" in datum:
                frate["nasoq-tuned_eps-6"] = failure_rate
            elif "nasoq-fixed" in datum:
                frate["nasoq-fixed_eps-6"] = failure_rate

    eps3_speedup = [frate["nasoq-fixed_eps-3"], frate["nasoq-tuned_eps-3"], frate["nasoq-custom_eps-3"]]
    eps6_speedup = [frate["nasoq-fixed_eps-6"], frate["nasoq-tuned_eps-6"], frate["nasoq-custom_eps-6"]]
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, np.array(eps3_speedup) * 100, width, label='eps-3')
    rects2 = ax.bar(x + width/2, np.array(eps6_speedup) * 100, width, label='eps-6')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('failure rate * 100')
    ax.set_title('failure rate * 100 of three tools with recommended settings')
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend()
    for tick in ax.get_xticklabels():
    ...
    plt.tight_layout()
    plt.savefig(plots_path + "different_solvers_frate.png")

def compute_failure_rate1():
    """
    failure rate of three tools with the same settings
    """
    data = os.listdir(path1)
    custom_data = []
    for datum in data:
    ...
    for datum in custom_data:
        ...
        if "eps-3" in datum:
            frate1["tested-3"] = failure_rate
            frate2["tested-3"] = failure_rate
        elif "eps-6" in datum:
            frate1["tested-6"] = failure_rate
            frate2["tested-6"] = failure_rate
    ...
    for datum in data1:
        if "nasoq-custom" in datum:
            ...
            if "eps-3" in datum:
                for diag_perturb in diag_perturb_lst:
                    if diag_perturb in datum and diag_perturb + "0" not in datum:
                        frate1["diag_perturb" + diag_perturb + "eps-3"] = failure_rate
            
            elif "eps-6" in datum:
                for diag_perturb in diag_perturb_lst:
                    if diag_perturb in datum and diag_perturb + "0" not in datum:
                        frate1["diag_perturb" + diag_perturb + "eps-6"] = failure_rate
    ...
    for datum in data2:
        if "nasoq-custom" in datum:
            ...
            if "eps-3" in datum:
                for max_iter in max_iter_lst:
                    if max_iter in datum and max_iter + "0" not in datum:
                        frate2["max_iter" + max_iter + "eps-3"] = failure_rate
            
            elif "eps-6" in datum:
                for max_iter in max_iter_lst:
                    if max_iter in datum and max_iter + "0" not in datum:
                        frate2["max_iter" + max_iter + "eps-6"] = failure_rate
    
    eps3_frate1 = [frate1["tested-3"], frate1["diag_perturb-12eps-3"], frate1["diag_perturb-11eps-3"], frate1["diag_perturb-10eps-3"], \
        frate1["diag_perturb-9eps-3"], frate1["diag_perturb-8eps-3"], frate1["diag_perturb-7eps-3"], frate1["diag_perturb-6eps-3"]]

    eps6_frate1 = [frate1["tested-6"], frate1["diag_perturb-12eps-6"], frate1["diag_perturb-11eps-6"], frate1["diag_perturb-10eps-6"], \
        frate1["diag_perturb-9eps-6"], frate1["diag_perturb-8eps-6"], frate1["diag_perturb-7eps-6"], frate1["diag_perturb-6eps-6"]]
    
    eps3_frate2 = [frate2["tested-3"], frate2["max_iter0eps-3"], frate2["max_iter1eps-3"], frate2["max_iter2eps-3"], frate2["max_iter3eps-3"], \
        frate2["max_iter4eps-3"], frate2["max_iter5eps-3"], frate2["max_iter10eps-3"], frate2["max_iter20eps-3"]]
    
    eps6_frate2 = [frate2["tested-6"], frate2["max_iter0eps-6"], frate2["max_iter1eps-6"], frate2["max_iter2eps-6"], frate2["max_iter3eps-6"], \
        frate2["max_iter4eps-6"], frate2["max_iter5eps-6"], frate2["max_iter10eps-6"], frate2["max_iter20eps-6"]]
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, np.array(eps3_frate1) * 100, width, label='eps-3')
    rects2 = ax.bar(x + width/2, np.array(eps6_frate1) * 100, width, label='eps-6')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('failure rate * 100')
    ax.set_title('failure rate * 100 of nasoq-custom \n(optimized vs non-optimized) for diag_perturb')
    ax.set_xticks(x)
    ax.set_xticklabels(labels1)
    ax.legend()
    for tick in ax.get_xticklabels():
    ...
    plt.savefig(plots_path + "different_diag_perturb_frate.png")
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, np.array(eps3_frate2) * 100, width, label='eps-3')
    rects2 = ax.bar(x + width/2, np.array(eps6_frate2) * 100, width, label='eps-6')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('failure rate * 100')
    ax.set_title('failure rate * 100 of nasoq-custom \n(optimized vs non-optimized) for max_iter')
    ax.set_xticks(x)
    ax.set_xticklabels(labels2)
    ax.legend()
    for tick in ax.get_xticklabels():
    ...
    plt.tight_layout()
    plt.savefig(plots_path + "different_max_iter_frate.png")
...
if __name__ == "__main__":
    ...
    compute_speedup1()
    compute_speedup2()
    compute_performance_profile1()
    compute_performance_profile2()
    compute_failure_rate1()
    compute_failure_rate2()
