...
/*		MSK_putdouparam(task,MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_DFEAS,1e-11);
		MSK_putdouparam(task,MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_PFEAS,1e-11);
		MSK_putdouparam(task,MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_INFEAS,1e-11);*/
		//MSK_putdouparam(task,MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_MU_RED,1e-11);
		return result;
	}).doNext([&]() {
...
double QuadProgMosek::getObjective() {
	double obj = 0;
	MSKtask_t task = __getMosekTask();
	if (task == NULL) {
		return obj;
...
int QuadProgMosek::getNumofIter() {
	int num_iter=0;
	MSKtask_t task = __getMosekTask();
	if (task == NULL) {
		return num_iter;
	}
	MSK_getintinf(task,MSK_IINF_INTPNT_ITER,&num_iter);
	return num_iter;
}

int QuadProgMosek::getNumofThread() {
	int num_thread=0;
	MSKtask_t task = __getMosekTask();
	if (task == NULL) {
		return num_thread;
	}
	MSK_getintinf(task,MSK_IINF_INTPNT_NUM_THREADS,&num_thread);
	return num_thread;
}
