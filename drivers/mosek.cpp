...
std::string  p_name = QPFC->smp_->desc_struct_.name_;
 ...
 double objective = 0;
 double *x, *u, *w;
 int num_iter=0, num_thread=0; //Mosek uses max threads by default


 // TODO remove all of these conversions.
 SparseMatrix<double> H_eigen(n,n);
 convert_mtx_to_eigen(n,n,QPFC->ief_->H->p,QPFC->ief_->H->i,
                      QPFC->ief_->H->x,
                      H_eigen);
 ...
 bool solve_state = mosekSolve(H_eigen,q_eig,B_eq_eig,b_eq_eig,A_eig,b_eig,
...
bool mosekSolve(const SparseMatrix<double>& MDK, const VectorXd& b,
                const SparseMatrix<double>& Aeq, const VectorXd& beq,
                const SparseMatrix<double>& Aineq, const VectorXd& bineq,
                VectorXd& v, VectorXd& dual_eq , VectorXd& dual_ineq,
                double &elapsed_time,
                const double eps, double &obj, int &num_iter,
                int &num_thread)
{
 ...
 program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_DFEAS,
                         eps);
 program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_PFEAS,
                         eps);
 program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_INFEAS,
                         eps);
/* program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_REL_GAP,
                         eps);*/
 program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_MU_RED,
                         eps);
 program->setParamDouble(MSKdparam_enum::MSK_DPAR_INTPNT_QO_TOL_NEAR_REL,
                         1.0);
 ...
 dual_eq = program->getDualEquality();
 dual_ineq = program->getDualInequality();
 obj = program->getObjective();
 num_iter = program->getNumofIter();
 num_thread = program->getNumofThread();
 return success;
}
