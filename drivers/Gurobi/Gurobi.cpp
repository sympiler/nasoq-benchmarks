...
int GurobiCommon::threads() const {
 return thread_;
}

 EIGEN_GUROBI_API void GurobiCommon::threads(int num_thread){
  thread_ = num_thread;
  model_.set(GRB_IntParam_Threads, num_thread);
}
...
const VectorXd& GurobiCommon::result() const
{
 ...
 const VectorXd& GurobiCommon::dualEqResult() const
 {
  return Yeq_;
 }

 const VectorXd& GurobiCommon::dualIneqResult() const
 {
  return Yineq_;
 }
	...
	const VectorXd& XL, const VectorXd& XU)
{
	...
	thread_ = model_.get(GRB_IntParam_Threads);
	...
	const VectorXd& XL, const VectorXd& XU)
{
 ...
 //FeasibilityTol https://www.gurobi.com/documentation/8.1/refman/feasibilitytol.html
	...
	bool success = fail_ == GRB_OPTIMAL;
	double* result = model_.get(GRB_DoubleAttr_X, vars_, nrvar_);
 double* eq_dual = model_.get(GRB_DoubleAttr_Pi, eqconstr_, nreq_);
 double* ineq_dual = model_.get(GRB_DoubleAttr_Pi, ineqconstr_, nrineq_);
	iter_ = model_.get(GRB_IntAttr_BarIterCount);
	X_ = Map<VectorXd>(result, nrvar_);
 Yeq_ = Map<VectorXd>(eq_dual, nreq_);
 Yineq_ = Map<VectorXd>(ineq_dual, nrineq_);
 thread_ = model_.get(GRB_IntParam_Threads);
	return success;
}
